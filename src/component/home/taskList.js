    ...
    return (
        <div className={homeStyle.taskListContainer}>
                ...
                (props?.taskList?.data) ?
                    props.taskList.data.map((task, i) => {
                        return (<div className={globalStyle.centerSpaceBetween + " " + homeStyle.taskList} key={i}>
                            <div className={globalStyle.centerSpaceBetween}>
                                <Link to={`http://localhost:3000/viewTask/${task._id}`} ><h2>{task.title}</h2></Link>
                                <p className={(task.state === "active") ? globalStyle.active : globalStyle.completed}>{task.state}</p>
                            </div>
                    ...
                    <h1 style={{marginTop : "20px" , textAlign : 'center', color : "white"}}>No Record Found!!!</h1>
            }
            {/* pagination */}
            {(props?.taskList?.data) && 
            <div className={globalStyle.centerSpaceBetween + " " + homeStyle.pagination}>
                ...
                <div>page {props.pageCount / 10}</div>
                <div className={homeStyle.paginationIcon} onClick={() => {
                    handelPage("right")
                }}><FaAngleRight /></div>
            </div>}
